...
} from '@/modules/reviews/presentation/dto/create-product-review.dto';
...
import type { ReviewItemProof } from '@/modules/reviews/application/types/review-order-context.type';
import type {
    ReviewMediaPurpose,
    ReviewMediaReference,
} from '@/modules/reviews/application/types/review-media-reference.type';
import {
    parseReviewMediaReference,
    uniqueReviewMediaReferences,
} from '@/modules/reviews/application/utils/review-media-reference.util';
import type { CleanupUploadedReviewMediaDto } from '@/modules/reviews/presentation/dto/cleanup-uploaded-review-media.dto';
...
export class ProductReviewService {
    ...
    async create(
        ...
        const proof = await this.orderReviewClient.getItemProof(
            dto.orderItemId,
        );
    ...
    async update(
        ...
        const removedMedia = this.collectRemovedReviewMedia(
            review,
            dto,
            userId,
        );
    ...
    async cleanupUploadedMedia(
        ...
        await this.reviewMediaClient.cleanupReviewAssets(
            userId,
            safeReferences,
        );
    }
    ...
    private assertReviewEligibility(
        ...
        if (
            proof.reviewDeadline &&
            new Date(proof.reviewDeadline) < new Date()
        ) {
    ...
    private parseReviewMediaUrls(
        ...
        if (
            references.some(
                (reference) => !reference || reference.ownerId !== userId,
            )
        ) {
            throw new BadRequestException(
                'Ảnh hoặc video đánh giá không hợp lệ.',
            );
        }
        ...
        if (!existing) {
            try {
                await this.likes.save(
                    this.likes.create({ reviewId, userId: ownerId }),
                );
            } catch (error) {
    ...
    private async requirePublicReview(
        reviewId: string,
    ): Promise<ProductReview> {
