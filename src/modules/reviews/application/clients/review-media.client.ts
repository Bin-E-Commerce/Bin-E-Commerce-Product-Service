...
import type { ReviewMediaReference } from '@/modules/reviews/application/types/review-media-reference.type';
    ...
    async cleanupReviewAssets(
        ...
        const response = await fetch(
            `${this.mediaServiceUrl}/api/v1/media/assets/review/cleanup`,
            {
                ...
                body: JSON.stringify({
                    assets: assets.map(({ assetId, purpose }) => ({
                        assetId,
                        purpose,
                    })),
                }),
            ...
            throw new BadGatewayException(
                'Không thể kết nối Media Service để dọn media review.',
            );
        });
