...
export class OrderReviewClient {
    ...
    async getItemProof(orderItemId: string): Promise<ReviewItemProof> {
        const response = await this.request(
            `/api/v1/internal/orders/items/${orderItemId}/review-context`,
        );
        ...
        if (!response.ok)
            throw new BadGatewayException(
                'Không thể xác minh đơn hàng lúc này.',
            );
    ...
    async getOrderContext(
        ...
        if (response.status === 401 || response.status === 403)
            throw new ForbiddenException(
                'Bạn không có quyền xem review của đơn hàng này.',
            );
        if (!response.ok)
            throw new BadGatewayException(
                'Không thể xác minh đơn hàng lúc này.',
