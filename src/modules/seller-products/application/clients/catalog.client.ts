...
export class CatalogClient {
    ...
    async getProductCategoryContext(categoryId: string): Promise<{
        ...
        if (
            ...
            attributeResponse.status === 404
        ) {
            throw new BadRequestException(
                'Ngành hàng đã chọn không còn hợp lệ.',
            );
        }
        ...
        return {
            category:
                (await categoryResponse.json()) as CatalogCategoryReference,
            attributes:
                (await attributeResponse.json()) as CatalogAttributeReference[],
        };
    }
