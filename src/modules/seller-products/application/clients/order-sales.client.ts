...
export class OrderSalesClient {
    ...
    async getSoldQuantities(
        ...
        try {
            ...
            if (!response.ok) {
                this.logger.warn(
                    `Order Service sales query failed with status ${response.status}`,
                );
                return null;
            }
            ...
            return new Map(
                rows.map((row) => [row.productId, Number(row.quantitySold)]),
            );
        } catch (error) {
            this.logger.warn(
                `Order Service sales query unavailable: ${String(error)}`,
            );
            return null;
        }
    }
