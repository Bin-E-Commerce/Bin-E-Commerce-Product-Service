...
import { parseProductMediaReference } from '@/modules/seller-products/application/utils/product-media-reference.util';
...
export class SellerProductCreationService {
    ...
    async create(
        ...
        if (dto.status === ProductStatus.ACTIVE) {
            await this.sellerShopClient.assertShippingReady(
                shopProfile.shop.id,
            );
        }
                    ...
                    await manager.save(
                        ...
                        dto.images.map((image) =>
                            manager.create(ProductImage, {
                                ...
                                altText:
                                    image.altText?.trim() || dto.name.trim(),
        ...
        const savedVariants = await manager.save(
            ...
            dto.variants.map((variant) => {
                ...
                return manager.create(ProductVariant, {
                    ...
                    withoutGtin:
                        !variant.gtin && !product.gtin && variant.withoutGtin,
