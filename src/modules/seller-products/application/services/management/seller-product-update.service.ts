...
import { CreateSellerProductDto } from '@/modules/seller-products/presentation/dto/create-product/create-seller-product.dto';
import {
    UpdateProductOptionDto,
    UpdateProductVariantDto,
    UpdateSellerProductDto,
} from '@/modules/seller-products/presentation/dto/update-product/update-seller-product.dto';
...
import type { CatalogAttributeReference } from '@/modules/seller-products/application/types/catalog-reference.type';
import type { ProductMediaReference } from '@/modules/seller-products/application/types/product-media-reference.type';
import type { SellerProductUserContext } from '@/modules/seller-products/application/types/seller-product-user-context.type';
import type { UpdateProductResponse } from '@/modules/seller-products/application/types/update-product-response.type';
import {
    parseProductMediaReference,
    uniqueProductMediaReferences,
} from '@/modules/seller-products/application/utils/product-media-reference.util';
...
export class SellerProductUpdateService {
    ...
    async update(
        ...
        try {
            const transactionResult = await this.dataSource.transaction(
                async (manager) => {
                    const product = await this.loadProductForUpdate(
                        manager,
                        productId,
                        currentUser.userId,
                    );
                    ...
                    const oldVideoReference = oldVideoAssetId
                        ? {
                              assetId: oldVideoAssetId,
                              purpose: 'product_video' as const,
                          }
                        : parseProductMediaReference(
                              product.videoUrl,
                              'product_video',
    ...
    private async replaceImages(
        ...
        const oldByUrl = new Map(
            oldImages.map((image) => [image.imageUrl.trim(), image]),
        );
        ...
        await manager.save(
            ...
            dto.images.map((image) => {
                ...
                const parsedSourceAssetId =
                    parseProductMediaReference(imageUrl, 'product_image')
                        ?.assetId ?? null;
                const sourceAssetId =
                    previous?.sourceAssetId ?? parsedSourceAssetId;
                return manager.create(ProductImage, {
                    ...
                    externalImageId:
                        previous?.externalImageId ?? parsedSourceAssetId,
                    sourceAssetId,
    ...
    private collectRemovedMedia(
        ...
        const newMedia = [
            ...
            dto.video?.assetId
                ? ({
                      assetId: dto.video.assetId,
                      purpose: 'product_video',
                  } satisfies ProductMediaReference)
                : null,
        ];
    ...
    private async filterSharedMedia(
        ...
        const sharedImageKeys = new Set(
            sharedImages
                ...
                .filter(
                    (media): media is ProductMediaReference => media !== null,
                )
        ...
        return staleMedia.filter((media) => {
            if (sharedImageKeys.has(`${media.purpose}:${media.assetId}`))
                return false;
            if (
    ...
    private async cleanupRemovedMedia(
        ...
        try {
            await this.productMediaClient.cleanupProductAssets(
                userId,
                staleMedia,
            );
        } catch (error) {
    ...
    private async replaceOptions(
        ...
        for (const variant of oldVariants) {
            await manager.delete(ProductVariantOptionValue, {
        ...
        const valueInputs = options.flatMap((option) =>
            option.values.map((value) => ({
                value,
                optionId: optionByClientId.get(option.clientId)!.id,
            })),
        );
    ...
    private async reconcileVariants(
        ...
        const oldById = new Map(
            oldVariants.map((variant) => [variant.id, variant]),
        );
        ...
        for (const input of variants) {
            ...
            entity.sku =
                variant?.sku ?? this.identifier.createSystemSku(shopId);
            ...
            entity.withoutGtin =
                !input.gtin && !product.gtin && input.withoutGtin;
            // Payload chỉnh sửa dùng tồn khả dụng; stockQuantity vẫn lưu tổng vật lý gồm cả lượng đang giữ chỗ.
            entity.stockQuantity = input.stockQuantity + reserved;
            entity.weight = (
                Number(product.packageWeightGrams) / 1_000
            ).toFixed(3);
            ...
            const inventory =
                variant?.inventory ??
                manager.create(Inventory, { variantId: savedVariant.id });
            inventory.variantId = savedVariant.id;
            inventory.quantityReserved = reserved;
            inventory.quantityAvailable = input.stockQuantity;
    ...
    private async replaceAttributes(
        ...
        await manager.save(
            ...
            dto.attributes.map((attribute) => {
                const catalogAttribute = catalogById.get(
                    attribute.categoryAttributeId,
                );
                if (!catalogAttribute)
                    throw new NotFoundException(
                        'Không tìm thấy thuộc tính ngành hàng.',
                    );
    ...
    private async findActiveBrand(brandId?: string): Promise<Brand | null> {
        ...
        if (!brand)
            throw new BadRequestException(
                'Thương hiệu đã chọn không còn hợp lệ.',
            );
        return brand;
    }
