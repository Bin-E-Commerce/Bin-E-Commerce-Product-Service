...
export class SellerProductAccessService {
    ...
    ensureCanReadProducts(
        ...
        if (!currentUser.permissions.includes(Permission.SELLER_PRODUCT_READ)) {
    ...
    ensureCanCreateProduct(
        ...
        if (
            !currentUser.permissions.includes(Permission.SELLER_PRODUCT_CREATE)
        ) {
            throw new ForbiddenException(
    ...
    ensureCanUpdateProduct(
        ...
        if (
            !currentUser.permissions.includes(Permission.SELLER_PRODUCT_UPDATE)
        ) {
            throw new ForbiddenException(
    ...
    ensureCanDeleteProduct(
        ...
        if (
            !currentUser.permissions.includes(Permission.SELLER_PRODUCT_DELETE)
        ) {
            throw new ForbiddenException(
    ...
    ensureCanChangeProductStatus(
        ...
        if (
            !currentUser.permissions.includes(
                Permission.SELLER_PRODUCT_STATUS_UPDATE,
            )
        ) {
            throw new ForbiddenException(
    ...
    ensureCanRestoreProduct(
        ...
        if (
            !currentUser.permissions.includes(Permission.SELLER_PRODUCT_RESTORE)
        ) {
            throw new ForbiddenException(
    ...
    ensureCanGenerateAiImage(
        currentUser: SellerProductUserContext,
    ): SellerProductUserContext {
        if (!currentUser.userId || !currentUser.email)
            throw new UnauthorizedException(
                'Ban can dang nhap de toi uu anh san pham.',
            );
        if (
            !currentUser.permissions.includes(
                Permission.SELLER_AI_IMAGE_OPTIMIZATION_GENERATE,
            )
        ) {
            throw new ForbiddenException(
                'Ban khong co quyen tao yeu cau toi uu anh.',
            );
        }
        return currentUser;
    }
    ...
    ensureCanApplyAiImage(
        currentUser: SellerProductUserContext,
    ): SellerProductUserContext {
        if (!currentUser.userId || !currentUser.email)
            throw new UnauthorizedException(
                'Ban can dang nhap de ap dung anh AI.',
            );
        if (
            !currentUser.permissions.includes(
                Permission.SELLER_AI_IMAGE_OPTIMIZATION_APPLY,
            )
        ) {
    ...
    ensureCanRollbackAiImage(
        currentUser: SellerProductUserContext,
    ): SellerProductUserContext {
        if (!currentUser.userId || !currentUser.email)
            throw new UnauthorizedException(
                'Ban can dang nhap de khoi phuc anh goc.',
            );
        if (
            !currentUser.permissions.includes(
                Permission.SELLER_AI_IMAGE_OPTIMIZATION_ROLLBACK,
            )
        ) {
            throw new ForbiddenException(
                'Ban khong co quyen khoi phuc anh goc.',
            );
        }
        return currentUser;
    }
