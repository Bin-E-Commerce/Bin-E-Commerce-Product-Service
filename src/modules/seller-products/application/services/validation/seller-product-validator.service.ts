...
} from '@/modules/seller-products/application/types/catalog-reference.type';
...
export class SellerProductValidatorService {
    ...
    private validateImages(dto: CreateSellerProductDto): void {
        ...
        const thumbnailCount = dto.images.filter(
    ...
    private validateAttributes(
        ...
        const selectedOptionIds = new Set(
            dto.attributes.flatMap(
                (attribute) => attribute.selectedOptionIds ?? [],
            ),
        );

        for (const submitted of dto.attributes) {
            const catalogAttribute = catalogById.get(
                submitted.categoryAttributeId,
            );
            if (!catalogAttribute) {
                throw new BadRequestException(
    ...
    private validateAttributeValue(
        ...
        const channelCount = [hasText, hasNumber, hasBoolean].filter(
            Boolean,
        ).length;
    ...
    private validateOptionsAndVariants(dto: CreateSellerProductDto): void {
        const optionNames = dto.options.map((option) =>
            option.name.trim().toLowerCase(),
        );
        const optionClientIds = dto.options.map((option) => option.clientId);
        this.assertUnique(
            optionNames,
            'Tên nhóm phân loại không được trùng nhau.',
        );
        this.assertUnique(
            optionClientIds,
            'Mã tạm của nhóm phân loại bị trùng.',
        );
            ...
            return dto.options
                .map((option) =>
                    variant.optionValueClientIds.find(
                        (valueId) =>
                            valueToOption.get(valueId) === option.clientId,
                    ),
                )
                .join('|');
        });
        this.assertUnique(
            combinations,
            'Tổ hợp variant không được trùng nhau.',
        );
    }
    ...
    private validateCommercialCodes(dto: CreateSellerProductDto): void {
        const gtins = [
            dto.gtin,
            ...dto.variants.map((variant) => variant.gtin),
