    ...
    implements OnModuleInit, OnModuleDestroy
{
    ...
    async publishForVariants(
        ...
        const variants = await (
            manager?.getRepository(ProductVariant) ?? this.variantRepository
        ).find({
    ...
    async publish(
        ...
        const event =
            await this.productRepository.manager.transaction(createEvent);
        if (!event) return;
        await this.outboxRepository.update(
            { eventId: event.eventId, status: 'PENDING' },
    ...
    private async dispatchEvent(
        event: RecommendationCatalogEvent,
    ): Promise<void> {
        ...
        if (published) {
            ...
            this.logger.debug(
                `Published catalog event ${event.eventName} for ${event.aggregateId}`,
            );
            return;
        }
      ...
      RETURNING outbox.event_id`,
        ...
        for (const row of pending) {
            await this.dispatchEvent(
                row.payload as unknown as RecommendationCatalogEvent,
            );
        }
    }
    ...
    private async dispatchPendingSafe(): Promise<void> {
        try {
            ...
            this.logger.warn(
                `Catalog outbox dispatch deferred: ${error instanceof Error ? error.message : 'unknown'}`,
            );
        }
    }
    ...
    private buildSemanticContent(
        product: Product,
    ): RecommendationCatalogEvent['data']['semanticContent'] {
