// Service này xây dựng read model danh sách product của seller theo shop scope và bộ lọc đã validate.
import {
    Injectable,
    NotFoundException,
...
} from '@/modules/seller-products/application/types/seller-product-list-response.type';
...
export class SellerProductsService {
    ...
    async getOwnedProductById(
        ...
        const likes = reviewIds.length
            ? await this.reviewLikeRepository.find({
        ...
        const reviewerProfiles =
            await this.reviewerProfileClient.getPublicProfiles(
                approvedReviews
        ...
        product.reviews = product.reviews.map((review) => {
            ...
            return {
                ...
                reviewerName: review.isAnonymous
                    ? null
                    : (review.reviewerName ?? profile?.name ?? null),
                reviewerAvatarUrl: review.isAnonymous
                    ? null
                    : (review.reviewerAvatarUrl ?? profile?.avatarUrl ?? null),
    ...
    private async loadSellerProductItems(
        ...
        const [reviewMetrics, soldQuantities] = await Promise.all([
            ...
            this.orderSalesClient.getSoldQuantities(sellerOwnerId, productIds),
        ]);
