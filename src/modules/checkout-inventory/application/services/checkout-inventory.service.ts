...
import type {
    ReleaseCheckoutDto,
    ReserveCheckoutDto,
} from '@/modules/checkout-inventory/presentation/dto/checkout-reservation.dto';
import type {
    CheckoutReservationResponse,
    CheckoutSnapshotItem,
} from '@/modules/checkout-inventory/application/types/checkout-response.type';
...
export class CheckoutInventoryService {
    ...
    async reserve(
        ...
        const result = await this.dataSource.transaction(async (manager) => {
            ...
            const reservationRepository =
                manager.getRepository(CheckoutReservation);
            ...
            if (existing)
                return {
                    response:
                        existing.response as unknown as CheckoutReservationResponse,
                };
            ...
            const inventories = await manager
                ...
                .where('inventory.variant_id IN (:...variantIds)', {
                    variantIds,
                })
    ...
    async release(dto: ReleaseCheckoutDto): Promise<{ released: boolean }> {
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            if (reservation?.status === CheckoutReservationStatus.RELEASED)
                return;
            ...
            await this.catalogEvents?.publishForVariants(
                releasedVariantIds,
                manager,
            );
        });
