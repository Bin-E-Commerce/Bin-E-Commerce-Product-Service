// File này là persistence model của catalog, sở hữu sản phẩm và các quan hệ cấu hình sản phẩm.
// Entity không chứa use case checkout, inventory hay HTTP; các domain đó chỉ liên kết qua relation TypeORM.
import {
    Column,
    CreateDateColumn,
    Entity,
...
@Index(['categoryId'])
@Index(['sellerShopId'])
@Index('IDX_products_public_shop_catalog', [
    'sellerShopId',
    'status',
    'originType',
])
